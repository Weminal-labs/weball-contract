module admin::gamev3 {
    ...
    public entry fun send_global_chat_message(
        ...
        assert!(player_account.tickets > 0, 1020); // Error code for no tickets
    ...
    public entry fun update_account(
       ...
       if (string::length(&new_username) > 0) {
           ...
           while (i < len) {
               ...
               assert!(account.username != new_username || account.address_id == player_address, E_USERNAME_ALREADY_EXISTS);
               i = i + 1;
           };
    ...
    public entry fun send_message_room_id(
        ...
        room_id: u64,
        message: String
    ) acquires RoomState, RoomChat, PlayerAccount {
        let sender_address = signer::address_of(sender);
        let username = get_player_username(sender_address);
        assert!(is_player_in_room(sender_address, room_id), E_NOT_AUTHORIZED);
    ...
    #[view]
    public fun get_top_10_players(): vector<PlayerData> acquires PlayerAccounts {
        get_top_n_players(10)
    }


    #[view]
    public fun get_top_50_players(): vector<PlayerData> acquires PlayerAccounts {
    ...
    fun get_top_n_players(n: u64): vector<PlayerData> acquires PlayerAccounts {
        let player_accounts = borrow_global<PlayerAccounts>(@admin);
        let accounts = &player_accounts.accounts;
        let len = vector::length(accounts);
